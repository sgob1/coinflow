const myTotal = function (amount, currency) {
  if (Number(amount) > 0) return `Your balance is ${prettyAmount(-amount)}${currency}`
  else if (Number(amount) === 0.0) return `Your balance is +${prettyAmount(0.0)}${currency}`
...
const myBalanceThisMonth = function (amount, currency) {
  if (Number(amount) > 0) return `This month your balance is +${prettyAmount(amount)}${currency}`
